...
celeb_classes = ["ben_afflek", "elton_john", "jerry_seinfeld", "madonna", "mindy_kaling"]

boost_classifier = pickle.load(open('./model-weights/test_gradient_boost_model.sav', 'rb'))

def _main():
    wind_name = 'face detection using MTCNN'
    cv2.namedWindow(wind_name, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(wind_name, 1280,720)
    ...
    while True:
        ...
        outs = detector.detect_faces(frame)

        # Remove the bounding boxes with low confidence
        faces = mtcnn_post_process(frame, outs, CONF_THRESHOLD)
        ...
        if len(faces) > 0:
            ...
            search_identities_facenet_boostbased(frame, faces, celeb_classes, boost_classifier, sess, embeddings, images_placeholder, phase_train_placeholder, face_aligner)        
